...
module.exports = function () {
  ...
  const Block = /*#__PURE__*/ (function () {
    function Block(_layout, _parent) {
      ...
      this._linePrependor = new this._config.linePrependor.fn(
        this._config.linePrependor.options,
      );
      this._lineAppendor = new this._config.lineAppendor.fn(
        this._config.lineAppendor.options,
      );
      this._blockPrependor = new this._config.blockPrependor.fn(
        this._config.blockPrependor.options,
      );
      this._blockAppendor = new this._config.blockAppendor.fn(
        this._config.blockAppendor.options,
      );
    }
    ...
    _createClass(Block, [
      {
        ...
        value: function _activate() {
          ...
          if (this._active) {
            throw Error(
              "This block is already active. This is probably a bug in RenderKid itself",
            );
          }

          if (this._closed) {
            throw Error(
              "This block is closed and cannot be activated. This is probably a bug in RenderKid itself",
            );
          }
      ...
      },
      {
        ...
        value: function _ensureActive() {
          if (!this._wasOpenOnce) {
            throw Error(
              "This block has never been open before. This is probably a bug in RenderKid itself.",
            );
          }

          if (!this._active) {
            throw Error(
              "This block is not active. This is probably a bug in RenderKid itself.",
            );
          }

          if (this._closed) {
            throw Error(
              "This block is already closed. This is probably a bug in RenderKid itself.",
            );
          }
        },
      },
      {
        ...
        value: function _open() {
          if (this._wasOpenOnce) {
            throw Error(
              "Block._open() has been called twice. This is probably a RenderKid bug.",
            );
          }
      ...
      },
      {
        ...
        value: function _writeLine(str) {
          let line,
            ...
            toAppendLength,
            toPrepend,
            toPrependLength; // we'll be cutting from our string as we go

          const remaining = new SpecialString(str);
          ...
          while (true) {
            ...
            roomLeft =
              this._layout._config.terminalWidth -
              (toPrependLength + toAppendLength); // how much room each line of content will have
