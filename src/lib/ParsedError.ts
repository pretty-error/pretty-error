let prop, _fn, _i, _len, _ref;

import * as sysPath from "node:path";

const ParsedError = (function () {
  ...
  ParsedError.prototype._parse = function () {
    ...
    if (typeof this.error !== "object") {
      ...
      if (this.error.kind != null) {
        ...
        if ((m = this._stack.match(/^([a-zA-Z0-9_$]+): /))) {
          this._kind = m[1];
        }
      }
      this._message =
  ...
  ParsedError.prototype._parseStack = function () {
    let line, message, messageLines, reachedTrace, _i, _len, _ref;
    messageLines = [];
    reachedTrace = false;
  ...
  ParsedError.prototype._rectifyPath = function (path, nameForCurrentPackage) {
    let m, packages, parts, remaining, rest;
    path = String(path);
    remaining = path;
